...
-->
## Upcoming Release
* Added `SecureVmGuestStateSas` as an optional parameter to the `Grant-AzDiskAccess` cmdlet. 
    - When the `SecureVmGuestStateSas` parameter is passed, the cmdlet requests to return `GrantAccessData.GetSecureVMGuestStateSAS Property`.
    - If this is for Non-Trusted launch or Confidential VM, the backend API will return an error `(BadRequest) GetSecureVMGuestStateSAS can be set only for disk with SecurityProfile.`
