...
-->
* Updated `Get-AzVmss` cmdlet:
    - Added validation to `ResourceGroupName` and `VMScaleSetName` parameters to throw an error if the parameter is used but is empty. This change is to avoid a bug where the cmdlet would return nothing if these parameters were empty.
